...
from PySide2.QtWidgets import QApplication, QWidget, QFileDialog, QToolButton,QStyle
...
class StringConfig(QWidget):
    ...
    @property
    def data(self) -> Model:    
        return  StringConfig.Model(value=self.value_spin.value(),
...
class TestConfig(QWidget):
    ...
    def __init__(self):
        ...
        self.path = QPathSelector(supported_types="*.lock",
                                 type=PathQuery.LoadSaveEnum.EXISTING_DIRECTORY)
    ...
    @property
    def data(self) -> Model:
        return TestConfig.Model(value1=self.value1.data,
                                value2=self.value2.data,
                                path=self.path.data,
                                influx=self.infux.data)
